...
w1BME280::w1BME280( void )
{
	...
	_hardPort = &Wire1; //Default to Wire port
...
//
//  This uses the stored w1BME280_SensorSettings to start the IMU
//  Use statements such as "mySensor.settings.commInterface = SPI_MODE;" to 
//  configure before calling .begin();
//
//****************************************************************************//
uint8_t w1BME280::begin()
{
	delay(2);  //Make sure sensor had enough time to turn on. w1BME280 requires 2ms to start up.
	...
	uint8_t chipID = readRegister(w1BME280_CHIP_ID_REG); //Should return 0x60 or 0x58
	if(chipID != 0x58 && chipID != 0x60) // Is this BMP or BME?
	return(chipID); //This is not BMP nor BME!

	//Reading all compensation data, range 0x88:A1, 0xE1:E7
	calibration.dig_T1 = ((uint16_t)((readRegister(w1BME280_DIG_T1_MSB_REG) << 8) + readRegister(w1BME280_DIG_T1_LSB_REG)));
	calibration.dig_T2 = ((int16_t)((readRegister(w1BME280_DIG_T2_MSB_REG) << 8) + readRegister(w1BME280_DIG_T2_LSB_REG)));
	calibration.dig_T3 = ((int16_t)((readRegister(w1BME280_DIG_T3_MSB_REG) << 8) + readRegister(w1BME280_DIG_T3_LSB_REG)));

	calibration.dig_P1 = ((uint16_t)((readRegister(w1BME280_DIG_P1_MSB_REG) << 8) + readRegister(w1BME280_DIG_P1_LSB_REG)));
	calibration.dig_P2 = ((int16_t)((readRegister(w1BME280_DIG_P2_MSB_REG) << 8) + readRegister(w1BME280_DIG_P2_LSB_REG)));
	calibration.dig_P3 = ((int16_t)((readRegister(w1BME280_DIG_P3_MSB_REG) << 8) + readRegister(w1BME280_DIG_P3_LSB_REG)));
	calibration.dig_P4 = ((int16_t)((readRegister(w1BME280_DIG_P4_MSB_REG) << 8) + readRegister(w1BME280_DIG_P4_LSB_REG)));
	calibration.dig_P5 = ((int16_t)((readRegister(w1BME280_DIG_P5_MSB_REG) << 8) + readRegister(w1BME280_DIG_P5_LSB_REG)));
	calibration.dig_P6 = ((int16_t)((readRegister(w1BME280_DIG_P6_MSB_REG) << 8) + readRegister(w1BME280_DIG_P6_LSB_REG)));
	calibration.dig_P7 = ((int16_t)((readRegister(w1BME280_DIG_P7_MSB_REG) << 8) + readRegister(w1BME280_DIG_P7_LSB_REG)));
	calibration.dig_P8 = ((int16_t)((readRegister(w1BME280_DIG_P8_MSB_REG) << 8) + readRegister(w1BME280_DIG_P8_LSB_REG)));
	calibration.dig_P9 = ((int16_t)((readRegister(w1BME280_DIG_P9_MSB_REG) << 8) + readRegister(w1BME280_DIG_P9_LSB_REG)));

	calibration.dig_H1 = ((uint8_t)(readRegister(w1BME280_DIG_H1_REG)));
	calibration.dig_H2 = ((int16_t)((readRegister(w1BME280_DIG_H2_MSB_REG) << 8) + readRegister(w1BME280_DIG_H2_LSB_REG)));
	calibration.dig_H3 = ((uint8_t)(readRegister(w1BME280_DIG_H3_REG)));
	calibration.dig_H4 = ((int16_t)((readRegister(w1BME280_DIG_H4_MSB_REG) << 4) + (readRegister(w1BME280_DIG_H4_LSB_REG) & 0x0F)));
	calibration.dig_H5 = ((int16_t)((readRegister(w1BME280_DIG_H5_MSB_REG) << 4) + ((readRegister(w1BME280_DIG_H4_LSB_REG) >> 4) & 0x0F)));
	calibration.dig_H6 = ((int8_t)readRegister(w1BME280_DIG_H6_REG));
	...
	return(readRegister(w1BME280_CHIP_ID_REG)); //Should return 0x60
}

//Begin comm with w1BME280 over SPI
bool w1BME280::beginSPI(uint8_t csPin)
{
...
//Begin comm with w1BME280 over I2C
bool w1BME280::beginI2C(TwoWire &wirePort)
{
	_hardPort = &wirePort;
	_wireType = HARD_WIRE;
...
//Begin comm with w1BME280 over software I2C
#ifdef SoftwareWire_h
bool w1BME280::beginI2C(SoftwareWire& wirePort)
{
	_softPort = &wirePort;
	_wireType = SOFT_WIRE;
...
// 01 and 10 = Forced
// 11 = Normal mode
void w1BME280::setMode(uint8_t mode)
{
	if(mode > 0b11) mode = 0; //Error check. Default to sleep mode
	
	uint8_t controlData = readRegister(w1BME280_CTRL_MEAS_REG);
	controlData &= ~( (1<<1) | (1<<0) ); //Clear the mode[1:0] bits
	controlData |= mode; //Set
	writeRegister(w1BME280_CTRL_MEAS_REG, controlData);
}
...
//  3, coefficients = 8
//  4, coefficients = 16
void w1BME280::setFilter(uint8_t filterSetting)
{
...
void w1BME280::setTempOverSample(uint8_t overSampleAmount)
{
...
//1 to 16 are valid over sampling values
void w1BME280::setPressureOverSample(uint8_t overSampleAmount)
{
	overSampleAmount = checkSampleValue(overSampleAmount); //Error check
...
//1 to 16 are valid over sampling values
void w1BME280::setHumidityOverSample(uint8_t overSampleAmount)
{
	overSampleAmount = checkSampleValue(overSampleAmount); //Error check
	...
	uint8_t controlData = readRegister(w1BME280_CTRL_HUMIDITY_REG);
	controlData &= ~( (1<<2) | (1<<1) | (1<<0) ); //Clear bits 2/1/0
	controlData |= overSampleAmount << 0; //Align overSampleAmount to bits 2/1/0
	writeRegister(w1BME280_CTRL_HUMIDITY_REG, controlData);
...
uint8_t w1BME280::checkSampleValue(uint8_t userValue)
{
	switch(userValue) 
	{
		case(0): 
...
//Default is 0x77
void w1BME280::setI2CAddress(uint8_t address)
{
...
bool w1BME280::isMeasuring(void)
{
	uint8_t stat = readRegister(w1BME280_STAT_REG);
	return(stat & (1<<3)); //If the measuring bit (3) is set, return true
}

//Strictly resets.  Run .begin() afterwards
void w1BME280::reset( void )
{
	writeRegister(w1BME280_RST_REG, 0xB6);
...
//Read all sensor registers as a burst. See w1BME280 Datasheet section 4. Data readout
//tempScale = 0 for Celsius scale (default setting)
//tempScale = 1 for Fahrenheit scale
void w1BME280::readAllMeasurements(w1BME280_SensorMeasurements *measurements, byte tempScale){
	
	uint8_t dataBurst[8];
	readRegisterRegion(dataBurst, w1BME280_MEASUREMENTS_REG, 8);
...
//
//****************************************************************************//
float w1BME280::readFloatPressure( void )
{

	// Returns pressure in Pa as unsigned 32 bit integer in Q24.8 format (24 integer bits and 8 fractional bits).
	// Output value of “24674867” represents 24674867/256 = 96386.2 Pa = 963.862 hPa
    uint8_t buffer[3];
	readRegisterRegion(buffer, w1BME280_PRESSURE_MSB_REG, 3);
...
void w1BME280::readFloatPressureFromBurst(uint8_t buffer[], w1BME280_SensorMeasurements *measurements)
{
...
void w1BME280::setReferencePressure(float refPressure)
{
	_referencePressure = refPressure;
}

//Return the local reference pressure
float w1BME280::getReferencePressure()
{
	return(_referencePressure);
}

float w1BME280::readFloatAltitudeMeters( void )
{
...
float w1BME280::readFloatAltitudeFeet( void )
{
	float heightOutput = 0;
...
//
//****************************************************************************//
float w1BME280::readFloatHumidity( void )
{
	
	// Returns humidity in %RH as unsigned 32 bit integer in Q22. 10 format (22 integer and 10 fractional bits).
	// Output value of “47445” represents 47445/1024 = 46. 333 %RH
    uint8_t buffer[2];
	readRegisterRegion(buffer, w1BME280_HUMIDITY_MSB_REG, 2);
...
void w1BME280::readFloatHumidityFromBurst(uint8_t buffer[], w1BME280_SensorMeasurements *measurements)
{
...
void w1BME280::setTemperatureCorrection(float corr)
{
	settings.tempCorrection = corr;
}

float w1BME280::readTempC( void )
{
    ...
    uint8_t buffer[3];
	readRegisterRegion(buffer, w1BME280_TEMPERATURE_MSB_REG, 3);
...
float w1BME280::readTempFromBurst(uint8_t buffer[])
{
...
void w1BME280::readTempCFromBurst(uint8_t buffer[], w1BME280_SensorMeasurements *measurements)
{
  measurements->temperature = readTempFromBurst(buffer);
}

float w1BME280::readTempF( void )
{
...
void w1BME280::readTempFFromBurst(uint8_t buffer[], w1BME280_SensorMeasurements *measurements)
{
...
double w1BME280::dewPointC(void)
{
...
double w1BME280::dewPointF(void)
{
...
//
//****************************************************************************//
void w1BME280::readRegisterRegion(uint8_t *outputPointer , uint8_t offset, uint8_t length)
{
...
uint8_t w1BME280::readRegister(uint8_t offset)
{
	//Return value
	uint8_t result = 0;
	uint8_t numBytes = 1;
...
int16_t w1BME280::readRegisterInt16( uint8_t offset )
{
	uint8_t myBuffer[2];
...
void w1BME280::writeRegister(uint8_t offset, uint8_t dataToWrite)
{
	switch (settings.commInterface)
	{
	case I2C_MODE:
